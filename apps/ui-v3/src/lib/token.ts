/**
 * The per-process CSRF token. The server injects
 * `<script>window.__APP_TOKEN__="..."</script>` before `</head>` when it
 * serves `apps/ui-v3/dist/index.html` under `--ui-v3`, so it exists in the
 * built app and is absent under `ui3-dev` (Vite serves its own index.html).
 *
